...
export default function MediaPreviewModal({
  ...
  const mediaUrl = currentMedia?.key
    ? `${process.env.NEXT_PUBLIC_S3_BASE_URL}/${currentMedia.key}`
    : "";
        ...
        {hasNavigation && (
          <ThumbnailBar>
            {allMedia.map((item, index) => {
              const thumbUrl = item.key
                ? `${process.env.NEXT_PUBLIC_S3_BASE_URL}/${item.key}`
                : "";
