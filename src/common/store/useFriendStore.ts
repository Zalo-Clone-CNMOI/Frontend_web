...
export const useFriendStore = create<FriendState>((set, get) => ({
 ...
 acceptFriendRequest: async (requestId) => {
  try {
    ...
    const request = get().pendingRequests.find((item) => item.id === requestId);
    ...
    if (friendUserId) {
      const res = await chatService.createConversation(friendUserId);
      const conversation = res?.payload?.data ?? res?.payload?.data ?? null;

      if (conversation) {
        useChatStore.getState().upsertConversationToTop(conversation);
      }
    }

    await Promise.all([get().fetchPendingRequests(), get().fetchFriends()]);
    return true;
  } catch (error: any) {
    set({
      error:
